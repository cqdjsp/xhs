...
#从手机中获取他们发的信息插入到数据库中，对转发中的转发进行处理
...
def process_time(input_time: str) -> str:
    ...
    time_short_pattern = r'^(\d{2}):(\d{2})$'           # 短时间：HH:MM 
 ...
 # 匹配短时间格式（HH:MM）  
    time_short_match = re.match(time_short_pattern, input_time.strip())  
    if time_short_match:  
        # 获取当前日期，并减去1天（前一天）  
        yesterday = datetime.now().date() - timedelta(days=1)  
        # 格式化为 "YYYY/MM/DD"（年为当前年，月日为前一天）  
        date_str = yesterday.strftime(f"{current_year}-%m-%d")  
        # 补充秒数为00  
        return f"{date_str} {input_time.strip()}:00" 
...
class WechatVersion:
    def __init__(self, version):
        ...
        self.ContentDHZF="com.tencent.mm:id/bkg"#群对话框里面的转发信息
        self.ContentDHText="com.tencent.mm:id/bkl"#群对话框里面的文本信息
        if(version=="8.0.54"):
            self.User="com.tencent.mm:id/lpa"#用户 
            self.Content="com.tencent.mm:id/lp8"#用户发的信息
            self.Time="com.tencent.mm:id/lp_"#发信息时间
...
if __name__ == "__main__":
    ...
    # #---------------------
    CFData=[]
    contentDHZF=d(resourceId=versionWC.ContentDHZF)
    for i in range(0,contentDHZF.count):
        allin=False
        contentDHZF[i].click()
        time.sleep(2)
        while(allin==False):
