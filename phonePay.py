...
class WeChatDonation:
    ...
    def scan_qrcode(self, qrcode,amount):
        """扫描赞赏码"""
        try:
            ...
            if not self.d(resourceId="com.tencent.mm:id/je0").exists(timeout=10):
               logger.error("未找到金额输入框")
               return False
            logger.info("点击相册按钮成功")
    ...
    def process_payments(self):
        ...
        for index, row in enumerate(self.data):
            ...
            if self.open_scanner():
                if self.scan_qrcode(qrcode,amount): 
                    returnvalue= self.confirm_payment(row)
                    if   returnvalue[0]: 
                        success_count += 1
...
if __name__ == "__main__":
    ...
    excel_path = "E:/my/job/xhs/Result/结算(10-10)2025_06_11_10_58_291.xls"  
